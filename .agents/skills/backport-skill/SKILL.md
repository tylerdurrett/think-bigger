---
name: backport-skill
description: Port a skill tweak made in a consumer repo back to the original tdog skills repo (the source of truth), then commit/push it there.
---

# Backport a skill change

Skills installed into a consumer repo under `.agents/skills/<name>/` are *copies*. The source of truth is the **tdog skills library**, identified portably by its git remote:
...
2. **`.tdog-skills-path`** — a gitignored file at the consumer repo's root containing the absolute path on one line. (Keep it gitignored; it's per-machine.)
...
If the consumer repo mirrors `.agents/skills/` into `.claude/skills/` via symlinks, you only ever edit one path on the consumer side.

## Procedure

1. Identify the change to port (the commit or working-tree diff in the consumer repo).
2. Resolve and verify the library checkout (above).
3. **Don't assume the two files are byte-identical.** Before applying, confirm the target's pre-change content matches what you edited in the consumer — grep the anchors / surrounding lines in the library file. If it has drifted, stop and reconcile by hand; do not blind-apply the same edits.
