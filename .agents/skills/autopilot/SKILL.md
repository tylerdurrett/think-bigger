...
Between audit (stage 2) and triage (stage 4) sits **one conditional sub-stage**: a surgical re-decompose that runs *only* when audit reports an in-boundary coverage gap needing its own task (the new-task path of [the severity gate](#the-severity-gate)). It dispatches `/decompose #<S>` to append the missing task and then hands into normal per-task triage. It does not renumber the six core stages or the skip-check table; its brief lives in [STAGES.md](STAGES.md#stage-35-surgical-re-decompose-coverage-gap-remedy-conditional).
...
The heart of the autonomy design. During the audit stage, every surviving finding is classified into one of **three** outcomes — routine (auto-apply, proceed), in-boundary coverage gap (auto-resolve, proceed), or blocking (halt):
...
- **In-boundary coverage gap** — an `ac-uncovered` / `ac-partial` finding: a Scope/AC bullet the slice *itself declared* has no covering task. This is NOT plan-poisoning — the batch still runs every other task correctly, and the missing piece is coherent and surfaces at the slice-PR review gate — so it does **NOT** halt. Resolve it by one of two paths, both non-halting:
  1. **Sibling-absorb (preferred when a natural home exists):** the gap is closable by a per-child `## Audit findings` body-edit note on an existing sibling task that is its natural home (e.g. widening an "integrations list" task to also cover the integration *detail* route). This is a per-child body edit — which the gate already calls ROUTINE — so audit lands the note itself and the run proceeds. No re-decompose.
  2. **New-task (when no existing sibling is a natural home):** the gap is a distinct cohesive chunk needing its own task. The orchestrator runs a **surgical re-decompose sub-stage** — re-invoke `/decompose #<S>`, which is additive on re-run (existing children preserved, matching titles skipped, only new children appended) — to append the missing task, then proceeds to triage. Still no halt.
- **Blocking / structural** — the plan is *plan-poisoning*: it corrupts more than the one child it names, so a batch built on it wastes a full parallel run. Exactly three categories:
  1. `oos-drift` — uncovered scope reaching **outside** the slice's declared boundary (not named or implied by its Scope/AC), so a human must re-scope. Contrast this with an `ac-uncovered` gap, which is *inside* the declared boundary: the **tell** is whether the uncovered scope is named or implied by the slice's own Scope/AC section.
  2. A `sequencing` reversal that breaks the DAG.
  3. `grounding` — hallucinated API surface that a task depends on.

When any blocking finding surfaces: **HALT**. Summarize the blocking finding(s) — claim, provenance, which child, why it's structural — and return control to the user. Do not proceed to the triage or batch stages. When genuinely unsure whether uncovered scope is an in-boundary gap (`ac-uncovered`) or a real boundary change (`oos-drift`), decide by the tell above — if the scope is named or implied by the slice's declared Scope, treat it as a coverage gap (cheap, reversible re-decompose or sibling note); only halt as `oos-drift` when the scope plausibly expands the slice's declared boundary. For the other blocking categories the general instinct still holds: when truly ambiguous, prefer the safe halt — a halt costs a re-run; a batch on a poisoned plan costs a full parallel execution.

**Why `sizing` is routine, not blocking.** An oversized task is *local to the one child it names and self-correcting at the promotion-PR review* — it costs at most one dense PR, never a wasted batch. That is the exact opposite of the three blocking categories, each of which poisons multiple tasks or the DAG. A `size:task` is *defined* as "one PR's worth," so a big-but-coherent task still executes in a single batch worktree and the human sees it whole in the review. Autopilot therefore records the audit's `sizing` note (the audit already lands it as a per-child body edit) and proceeds — it never halts to split a task. Splitting is not audit's job to begin with: audit's slice mode creates **no** children — "slice-level gaps are a `/decompose` re-decompose problem; task is a leaf." If automated splitting is ever wanted, its home is a *surgical slice re-decompose in `/decompose`* (a task is a leaf, so there is no "decompose the task" — an oversized task means the slice→task cut was slightly wrong), invoked as its own sub-stage between audit and triage — never in audit, and never as a halt. That same sub-stage — a surgical `/decompose #<S>` re-run between audit and triage — is what actually runs for the **new-task coverage-gap path** above; it is the doc's single home for any additive slice→task correction (a missing-coverage task, or a hypothetical split), and it only ever appends, never halts.

Why routine findings can auto-proceed: the downstream safety net. Every task inside `/batch` gets an independent code review, and the run ends in a slice promotion PR the user reviews before anything reaches the feature branch. The severity halt doesn't exist to protect correctness — the PR gate does that — it exists to avoid wasting a full parallel batch run on a structurally broken plan. An in-boundary coverage gap is *incomplete-but-coherent*, not structurally broken: the rest of the batch is unaffected and the added or absorbed coverage surfaces at that same PR gate — so it resolves in-flight (a sibling note, or the surgical re-decompose sub-stage) rather than halting.

**Non-blocking checkpoint.** Whether or not anything blocked, emit a short summary at the audit boundary — findings counts, what was auto-applied, provenance, and any in-boundary coverage gaps with how each was routed (sibling-absorbed into #<sibling>, or a surgical re-decompose that appended #<X>) — before dispatching stage 4. A user who is watching can interrupt there; one who isn't is not required to.
...
- It does not reimplement any stage. It dispatches the existing skills and relays their reports — the conditional surgical re-decompose sub-stage (the coverage-gap remedy) likewise only *dispatches* `/decompose #<S>` to append a missing-coverage task; it never re-decomposes or splits tasks itself.
